...
class ReviewRequest(BaseModel):
    ...
    provider: str = Field(
        ..., description="Git server provider (github, gitlab, bitbucket)"
    )
    ...
    ai_model: Optional[str] = Field(
        None, description="AI model to use (overrides config)"
    )
    max_comments: Optional[int] = Field(
        None, description="Maximum inline comments to post"
    )
...
class ReviewResponse(BaseModel):
    ...
    quality_score: Optional[Dict[str, int]] = Field(
        None, description="Code quality scoring"
    )
    ...
    comments_posted: Optional[int] = Field(
        None, description="Number of comments posted"
    )
...
async def validate_config():
    ...
    if not config_manager:
        try:
            ...
            return ConfigResponse(
                valid=False, errors=[f"Failed to load configuration: {str(e)}"]
            )
    ...
    try:
        ...
        return ConfigResponse(
            valid=len(errors) == 0, errors=errors, providers_available=providers
        )
    except Exception as e:
        return ConfigResponse(
            valid=False, errors=[f"Configuration validation failed: {str(e)}"]
        )
...
async def review_pr(request: ReviewRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if not adapter.validate_credentials():
            raise HTTPException(
                status_code=401, detail=f"Invalid credentials for {request.provider}"
            )
...
if __name__ == "__main__":
    ...
    uvicorn.run("web_api:app", host=host, port=port, reload=False, access_log=True)
